# add near top (after your imports)
...
# If you want a short description at top
st.sidebar.markdown("Select a page to view Pacers / Spinners analysis")

# Then at the place where you render charts, wrap by page:
...
# =========================================================
    ...
    render_batters_page()  # implement this by factoring existing Batters rendering block into a function
elif page == "Pacers":
    # show only pacers (call your functions on df_seam or filtered pacers df)
    st.header("Pacers Dashboard")
    # You can reuse functions from your file: create_crease_beehive, create_pitch_map, etc.
    df_pacers = df_raw[df_raw["BowlerType"].str.lower().str.contains("pacer|seam", na=False)]
    st.plotly_chart(create_crease_beehive(df_pacers, "Seam"), use_container_width=True)
    st.pyplot(create_pitch_map(df_pacers, "Seam"), use_container_width=True)
    # add other Pacers-specific charts...
elif page == "Spinners":
    st.header("Spinners Dashboard")
    df_spinners = df_raw[df_raw["BowlerType"].str.lower().str.contains("spin", na=False)]
    st.plotly_chart(create_crease_beehive(df_spinners, "Spin"), use_container_width=True)
    st.pyplot(create_pitch_map(df_spinners, "Spin"), use_container_width=True)
    # add other Spinners-specific charts...
